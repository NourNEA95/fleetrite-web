...
class KocDirectorateReportService
{
    /**
     * Generate report by calling legacy API (parallel, batch of 1).
     * User requested 100 imei request parallel, which we handle by sending 
     * multiple requests from the frontend or here. For this modular pattern, 
     * the frontend handles the 100 parallel calls to the /generate endpoint.
     */
    public function generate(array $params, int $userId)
    {
        ...
        $postData = [
            ...
            'dialog_report_type' => 'overspeednew_save',
            'dialog_report_data_item_list' => 'time,start,end,move_duration,route_length', // Standard for this report type
        ...
        try {
            ...
            Log::error("KOC Directorate Service Exception: " . $e->getMessage());
        ...
        try {
            ...
            $transformed = $results->map(function ($row) {
                return [
                    ...
                    'location' => $row->location . ($row->user_zone && $row->user_zone != 'n/a' ? ' - ' . $row->user_zone : ''),
                    'heading' => $row->heading_direction,
                    'recorded_speed' => $row->recorded_speed,
                    'speed_limit' => $row->allowed_speed_limit,
                    'start_time' => $row->start_time,
                    'end_time' => $row->end_time,
                    'speed' => $row->speed,
                    'os_duration' => $row->os_duration,
            ...
            Log::error("KocDirectorateReportService Fetch Error: " . $e->getMessage());
            throw $e;
        }
    }
}
