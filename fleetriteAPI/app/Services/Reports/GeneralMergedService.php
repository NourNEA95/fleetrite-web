...
class GeneralMergedService
{
    /**
     * Generate modular merged report using legacy process_api.php
     */
    public function generate(array $params)
    {
        ...
        $legacyParams = [
            ...
            'dialog_report_type' => 'general_merged_save',
            'type' => 'General Information (Merged)',
        ...
        foreach (array_chunk($imeiChunks, $maxParallel) as $batch) {
            ...
            foreach ($responses as $index => $response) {
                $imei = $imeiChunks[$index][0] ?? 'unknown';
                if ($response->successful()) {
                    $body = $response->json();
                    if (isset($body['key'])) {
                        $keys[] = $body['key'];
                        $results[] = ['imei' => $imei, 'status' => 'success', 'key' => $body['key']];
                    } else {
                        $results[] = ['imei' => $imei, 'status' => 'error', 'key' => null, 'message' => 'No key in response', 'body' => $body];
                    }
                } else {
                    $results[] = ['imei' => $imei, 'status' => 'error', 'key' => null, 'message' => 'HTTP error: ' . $response->status()];
                }
            }
        }
    ...
    public function fetchData($keysParam, int $page, int $rows, int $id = null)
    {
        ...
        $allData = DB::table('general_information_reports2_api')
            ->whereIn('table_key', $keys)
            ->where('type', 0)
            ->get();
        ...
        foreach ($allData as $row) {
            ...
            $totals['engine_work'] += (int) $row->engine_work; // assuming time string is handled by frontend or already converted
